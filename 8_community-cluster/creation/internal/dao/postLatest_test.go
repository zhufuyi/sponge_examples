...
func Test_postLatestDao_GetByID(t *testing.T) {
	...
	_, err := d.IDao.(PostLatestDao).GetByID(d.Ctx, testData.ID) // notfound
	if err != nil {
		t.Fatal(err)
	}
...
func Test_postLatestDao_CreateByTx(t *testing.T) {
	...
	err := d.IDao.(PostLatestDao).CreateByTx(d.Ctx, d.DB, testData)
	if err != nil {
		t.Fatal(err)
	}
}
...
func Test_postLatestDao_DeleteByTx(t *testing.T) {
	...
	err := d.IDao.(PostLatestDao).DeleteByTx(d.Ctx, d.DB, testData.ID, 1)
	if err != nil {
		t.Fatal(err)
	}
}
