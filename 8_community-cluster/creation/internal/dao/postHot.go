...
type PostHotDao interface {
	...
	GetByCondition(ctx context.Context, condition *query.Conditions) (*model.PostHot, error)
	...
	CreateByTx(ctx context.Context, tx *gorm.DB, table *model.PostHot) (uint64, error)
	DeleteByTx(ctx context.Context, tx *gorm.DB, id uint64, delFlag int) error
	UpdateByTx(ctx context.Context, tx *gorm.DB, table *model.PostHot) error
}
...
func NewPostHotDao(db *gorm.DB, xCache cache.PostHotCache) PostHotDao {
	return &postHotDao{
		db:    db,
		cache: xCache,
...
func (d *postHotDao) updateDataByID(ctx context.Context, db *gorm.DB, table *model.PostHot) error {
	if table.ID < 1 {
...
//		},
//	}
func (d *postHotDao) GetByCondition(ctx context.Context, c *query.Conditions) (*model.PostHot, error) {
...
func (d *postHotDao) CreateByTx(ctx context.Context, tx *gorm.DB, table *model.PostHot) (uint64, error) {
	err := tx.WithContext(ctx).Create(table).Error
	return table.ID, err
}

// DeleteByTx delete a record in the database using the provided transaction
func (d *postHotDao) DeleteByTx(ctx context.Context, tx *gorm.DB, id uint64, delFlag int) error {
	update := map[string]interface{}{
		"del_flag":   delFlag,
		"deleted_at": time.Now(),
	}
	err := tx.WithContext(ctx).Model(&model.PostHot{}).Where("post_id = ?", id).Updates(update).Error
	if err != nil {
		return err
	}
...
func (d *postHotDao) UpdateByTx(ctx context.Context, tx *gorm.DB, table *model.PostHot) error {
