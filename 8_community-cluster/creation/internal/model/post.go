...
type Post struct {
	...
	PostType     int     `gorm:"column:post_type;type:tinyint(4);NOT NULL" json:"postType"`               // 类型：0:未知, 1:文本, 2:图片, 3:视频
	...
	ViewCount    uint    `gorm:"column:view_count;type:int(10) unsigned;NOT NULL" json:"viewCount"`       // 查看帖子次数
	LikeCount    uint    `gorm:"column:like_count;type:int(10) unsigned;NOT NULL" json:"likeCount"`       // 点赞数
	CommentCount uint    `gorm:"column:comment_count;type:int(10) unsigned;NOT NULL" json:"commentCount"` // 评论数
	CollectCount uint    `gorm:"column:collect_count;type:int(10) unsigned;NOT NULL" json:"collectCount"` // 收藏数
	ShareCount   uint    `gorm:"column:share_count;type:int(10) unsigned;NOT NULL" json:"shareCount"`     // 分享数
