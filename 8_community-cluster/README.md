...
<p align="center">Figure 2 grpc gateway service code structure diagram</p>

As can be seen from Figure 2, developing a complete grpc gateway service focuses on **defining api interfaces** and **writing specific business logic code in template code**. Among them, **defining api interfaces** already exists in monolithic web services [community-single](https://github.com/zhufuyi/sponge_examples/tree/main/7_community-single), no need to rewrite, just copy proto files over.
...
In grpc gateway services, if you need to assemble data from multiple microservices into a new api interface, fill in the description information of this assembled new api interface into community_gw.proto file.
...
Enter the sponge UI interface, click on the left menu bar 【Protobuf】 -> 【create microservice project】, fill in the parameters, and generate three microservice codes separately.

#### Create user service

This is the proto file [user.proto](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/user/api/user/v1/user.proto) copied from the monolithic service community-single, which is used to quickly generate user service code, as shown below:
...
This is the proto file [relation.proto](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/relation/api/relation/v1/relation.proto) copied from the monolithic service community-single, which is used to quickly generate relation service code, as shown below:
...
This command generates service template code, error codes, grpc client test code. These codes correspond to Figure 1 egg white part.

- **Service template code**, in the `internal/service` directory. The method functions in the file correspond one-to-one with the grpc method names defined in the proto file. By default, there are simple usage examples under each method function. Just write specific logic code in each method function.
- **Interface error codes**, in the `internal/ecode` directory. The file name is consistent with the proto file name and the suffix is `_rpc.go`. The default error code variables in the file correspond one-to-one with the grpc method names defined in the proto file. Add or change business-related error codes here. Note that error codes cannot be repeated, otherwise panic will be triggered.
- **grpc client test code**, in `internal/service` directory. The file name is consistent with the proto file name and suffix is `_client_test.go`. The method functions in this file correspond one-to-one with grpc method names defined in proto files. Fill in parameters for each grpc method.
...
**The third step is to migrate specific logic code**. Copy specific logic code under each method function of monolithic web service community-single's `internal/handler/user.go` into same-named functions of user service's `internal/service/user.go`. These codes are the part of writing business logic code in Figure 1 egg yolk.
...
### Test grpc methods of user, relation, creation services

#### Test grpc methods of user service

After writing the business logic code, start the service to test the grpc method. Before starting the service for the first time, open the configuration file (`user/configs/user.yml`) to set mysql and redis addresses, set grpc and grpcClient related parameters, and then execute the command to compile and start the service:
...
Open user service code in goland IDE, enter `user/internal/service` directory, find code files with suffix `_client_test.go`, fill in parameters for each grpc method and test.

<br>

#### Test grpc methods of relation service

To test grpc methods of relation service, please refer to testing grpc methods of user service above.

<br>

#### Test grpc methods of creation service

To test grpc methods of creation service, please refer to testing grpc methods of user service above.

<br>

## grpc gateway service

After completing the user, relation, and creation services, the grpc gateway service community_gw needs to be completed next. Community_gw serves as the unified entrance for user, relation, and creation services.

### Create community_gw service

Enter the sponge UI interface, click on the left menu bar 【Protobuf】 -> 【Create grpc gateway project】, fill in some parameters to generate grpc gateway service code.

The proto files([user_gw.proto](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/api/community_gw/v1/user_gw.proto)、[relation_gw.proto](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/api/community_gw/v1/relation_gw.proto)、[post_gw.proto](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/api/community_gw/v1/post_gw.proto)、[comment_gw.proto](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/api/community_gw/v1/comment_gw.proto)、[like_gw.proto](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/api/community_gw/v1/like_gw.proto)、[collect_gw.proto](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/api/community_gw/v1/collect_gw.proto)) copied from the monolithic service community-single to quickly create the grpc gateway service community_gw, as shown below:
...
Because community_gw service serves as the request entrance and communicates with user, relation, and creation services using grpc method, it is necessary to generate code that connects to user, relation, and creation services. Enter the sponge UI interface, click on the left menu bar 【Public】 -> 【Generate grpc connection code】, fill in some parameters to generate grpc service connection code, as shown below:
...
At the same time, copy the proto files of user, relation, and creation services to the api directory of community_gw.

```bash
make copy-proto SERVER=../user,../relation,../creation
```

Through simple operations, we have completed creating grpc gateway service community_gw.
...
(1) **Generated service template code**, in `community_gw/internal/service` directory, the names are:

[collect_gw.go](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/internal/service/collect_gw.go), [comment_gw.go](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/internal/service/comment_gw.go), [like_gw.go](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/internal/service/like_gw.go), [post_gw.go](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/internal/service/post_gw.go), [relation_gw.go](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/internal/service/relation_gw.go), [user_gw.go](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/internal/service/user_gw.go)

In these files, method functions correspond one-to-one with grpc method names defined in proto files. There are default usage examples under each method function. Just make simple adjustments to call grpc methods of user, relation, and creation services. The above files are codes after writing specific logic.

<br>

(2) **Generated registration route code**, in `community_gw/internal/routers` directory. The file name is consistent with proto file name and suffix is `_router.go`. The names are:

[collect_gw_router.go](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/internal/routers/collect_gw_router.go), [comment_gw_router.go](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/internal/routers/comment_gw_router.go), [like_gw_router.go](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/internal/routers/like_gw_router.go), [post_gw_router.go](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/internal/routers/post_gw_router.go), [relation_gw_router.go](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/internal/routers/relation_gw_router.go), [user_gw_router.go](https://github.com/zhufuyi/sponge_examples/blob/main/8_community-cluster/community_gw/internal/routers/user_gw_router.go)
...
In these files, default error code variables correspond one-to-one with grpc method names defined in proto files. Add or change business-related error codes here. Note that error codes cannot be repeated otherwise panic will be triggered.

Note: If the called grpc method itself contains an error code, it can be returned directly.
...
After writing the business logic code, start the service to test the api interface. Before starting the service for the first time, open the configuration file (`community_gw/configs/community_gw.yml`) to set the connection grpc service configuration information as shown below:
...
- For grpc gateway service community-gw, add your own plug-ins (gin middleware) in the code file `community-gw/internal/routers/routers.go`.
