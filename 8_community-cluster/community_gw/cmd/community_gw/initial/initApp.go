...
func Config() {
	...
	rpcclient.NewUserRPCConn()
	rpcclient.NewRelationRPCConn()
	rpcclient.NewCreationRPCConn()
...
func initConfig() {
	...
	if enableConfigCenter {
		...
		if configFile == "" {
			configFile = configs.Path("community_gw_cc.yml")
		}
		...
		if configFile == "" {
			configFile = configs.Path("community_gw.yml")
		}
