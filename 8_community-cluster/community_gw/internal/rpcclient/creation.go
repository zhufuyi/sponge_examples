...
var (
	creationConn *grpc.ClientConn
	creationOnce sync.Once
)

// NewCreationRPCConn instantiate rpc client connection
func NewCreationRPCConn() {
	cfg := config.Get()
	...
	var err error
	creationConn, err = grpccli.Dial(context.Background(), endpoint, cliOptions...)
	if err != nil {
		panic(fmt.Sprintf("dial rpc server failed: %v, name: %s, endpoint: %s", err, serverName, endpoint))
	}
}

// GetCreationRPCConn get client conn
func GetCreationRPCConn() *grpc.ClientConn {
	if creationConn == nil {
...
// CloseCreationRPCConn Close tears down the ClientConn and all underlying connections.
func CloseCreationRPCConn() error {
	if creationConn == nil {
		return nil
	}

	return creationConn.Close()
}
