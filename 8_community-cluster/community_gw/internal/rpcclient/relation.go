...
var (
	relationConn *grpc.ClientConn
	relationOnce sync.Once
)

// NewRelationRPCConn instantiate rpc client connection
func NewRelationRPCConn() {
	cfg := config.Get()
	...
	var err error
	relationConn, err = grpccli.Dial(context.Background(), endpoint, cliOptions...)
	if err != nil {
		panic(fmt.Sprintf("dial rpc server failed: %v, name: %s, endpoint: %s", err, serverName, endpoint))
	}
}

// GetRelationRPCConn get client conn
func GetRelationRPCConn() *grpc.ClientConn {
	if relationConn == nil {
...
// CloseRelationRPCConn Close tears down the ClientConn and all underlying connections.
func CloseRelationRPCConn() error {
	if relationConn == nil {
		return nil
	}

	return relationConn.Close()
}
