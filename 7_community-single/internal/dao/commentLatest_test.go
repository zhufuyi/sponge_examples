...
func Test_commentLatestDao_GetByID(t *testing.T) {
	...
	_, err := d.IDao.(CommentLatestDao).GetByID(d.Ctx, testData.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func Test_commentLatestDao_GetByCondition(t *testing.T) {
	d := newCommentLatestDao()
	defer d.Close()
	testData := d.TestData.(*model.CommentLatest)
	...
	_, err := d.IDao.(CommentLatestDao).GetByCondition(d.Ctx, &query.Conditions{
...
func Test_commentLatestDao_DeleteByTx(t *testing.T) {
	...
	err := d.IDao.(CommentLatestDao).DeleteByTx(d.Ctx, d.DB, testData.ID)
	if err != nil {
		t.Fatal(err)
	}
}
