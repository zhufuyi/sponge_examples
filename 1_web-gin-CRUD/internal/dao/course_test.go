...
func Test_courseDao_GetByID(t *testing.T) {
	...
	_, err := d.IDao.(CourseDao).GetByID(d.Ctx, testData.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func Test_courseDao_GetByCondition(t *testing.T) {
	d := newCourseDao()
	defer d.Close()
	testData := d.TestData.(*model.Course)
	...
	_, err := d.IDao.(CourseDao).GetByCondition(d.Ctx, &query.Conditions{
