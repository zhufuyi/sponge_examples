...
// Test_service_downgrade_methods is used to test the downgrade api
// Test_service_downgrade_benchmark is used to performance test the downgrade api
...
// Test service downgrade api via grpc client
func Test_service_downgrade_methods(t *testing.T) {
	conn := getRPCClientConnForTest()
	cli := stockV1.NewDowngradeClient(conn)
	...
	tests := []struct {
...

		{
			...
			fn: func() (interface{}, error) {
				...
				req := &stockV1.UpdateDowngradeRequest{
					Id:    0,
					Stock: 0, // 库存数量
				}
...

		{
			...
			fn: func() (interface{}, error) {
				// todo type in the parameters before testing
				req := &stockV1.QueryDowngradeRequest{
					Id: 0,
				}
		...
		{
			name: "DowngradeBranch",
			fn: func() (interface{}, error) {
				// todo type in the parameters before testing
				req := &stockV1.DowngradeBranchRequest{
					Gid:   "", // dtm gid
					Key:   "", // 缓存key
					Id:    0,
					Stock: 0, // 库存数量
				...
				return cli.DowngradeBranch(ctx, req)
			},
			wantErr: false,
		},
	}
...
// performance test service downgrade api, copy the report to
// the browser to view when the pressure test is finished.
func Test_service_downgrade_benchmark(t *testing.T) {
	...
	protoFile := configs.Path("../api/stock/v1/downgrade.proto")
	...
	tests := []struct {
...

		{
			...
			fn: func() error {
				...
				message := &stockV1.UpdateDowngradeRequest{
					Id:    0,
					Stock: 0, // 库存数量
				}
...

		{
			...
			fn: func() error {
				// todo type in the parameters before benchmark testing
				message := &stockV1.QueryDowngradeRequest{
					Id: 0,
				}
				total := 1000 // total number of requests
...

		{
			...
			fn: func() error {
				// todo type in the parameters before benchmark testing
				message := &stockV1.DowngradeBranchRequest{
					Gid:   "", // dtm gid
					Key:   "", // 缓存key
					Id:    0,
					Stock: 0, // 库存数量
				...
				b, err := benchmark.New(host, protoFile, "DowngradeBranch", message, dependentProtoFilePath, total)
				if err != nil {
					return err
				}
				return b.Run()
