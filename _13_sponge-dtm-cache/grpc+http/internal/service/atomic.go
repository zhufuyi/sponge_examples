...
func init() {
	registerFns = append(registerFns, func(server *grpc.Server) {
		stockV1.RegisterAtomicServer(server, NewAtomicServer())
	})
}

var _ stockV1.AtomicServer = (*atomic)(nil)

type atomic struct {
	stockV1.UnimplementedAtomicServer

	db          *sql.DB
	cacheClient *rockscache.Client
}

// NewAtomicServer create a server
func NewAtomicServer() stockV1.AtomicServer {
	return &atomic{
		db:          model.GetSDB(),
...
func (s *atomic) Update(ctx context.Context, req *stockV1.UpdateAtomicRequest) (*stockV1.UpdateAtomicRequestReply, error) {
	err := req.Validate()
	if err != nil {
...
// Query  查询
func (s *atomic) Query(ctx context.Context, req *stockV1.QueryAtomicRequest) (*stockV1.QueryAtomicReply, error) {
	err := req.Validate()
	if err != nil {
		logger.Warn("req.Validate error", logger.Err(err), logger.Any("req", req), interceptor.ServerCtxRequestIDField(ctx))
