...
// Test_service_atomic_methods is used to test the atomic api
// Test_service_atomic_benchmark is used to performance test the atomic api
...
// Test service atomic api via grpc client
func Test_service_atomic_methods(t *testing.T) {
	conn := getRPCClientConnForTest()
	cli := stockV1.NewAtomicClient(conn)
	...
	tests := []struct {
...

		{
			...
			fn: func() (interface{}, error) {
				...
				req := &stockV1.UpdateAtomicRequest{
					Id:    1,
					Stock: 100, // 库存数量
				}
...

		{
			...
			fn: func() (interface{}, error) {
				// todo type in the parameters before testing
				req := &stockV1.QueryAtomicRequest{
					Id: 1,
				}
...
// performance test service atomic api, copy the report to
// the browser to view when the pressure test is finished.
func Test_service_atomic_benchmark(t *testing.T) {
	...
	protoFile := configs.Path("../api/stock/v1/atomic.proto")
	...
	tests := []struct {
...

		{
			...
			fn: func() error {
				...
				message := &stockV1.UpdateAtomicRequest{
					Id:    0,
					Stock: 0, // 库存数量
				}
...

		{
			...
			fn: func() error {
				// todo type in the parameters before benchmark testing
				message := &stockV1.QueryAtomicRequest{
					Id: 0,
				}
				total := 1000 // total number of requests
