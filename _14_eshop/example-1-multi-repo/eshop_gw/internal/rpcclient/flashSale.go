...
import (
	...
	"eshop_gw/internal/config"
)

var (
	flashSaleConn *grpc.ClientConn
	flashSaleOnce sync.Once
)

// NewFlashSaleRPCConn instantiate rpc client connection
func NewFlashSaleRPCConn() {
	cfg := config.Get()

	serverName := "flashSale"
	...
	var err error
	flashSaleConn, err = grpccli.Dial(context.Background(), endpoint, cliOptions...)
	if err != nil {
		panic(fmt.Sprintf("dial rpc server failed: %v, name: %s, endpoint: %s", err, serverName, endpoint))
	}
}

// GetFlashSaleRPCConn get client conn
func GetFlashSaleRPCConn() *grpc.ClientConn {
	if flashSaleConn == nil {
		flashSaleOnce.Do(func() {
			NewFlashSaleRPCConn()
		})
	}

	return flashSaleConn
}

// CloseFlashSaleRPCConn Close tears down the ClientConn and all underlying connections.
func CloseFlashSaleRPCConn() error {
	if flashSaleConn == nil {
		return nil
	}

	return flashSaleConn.Close()
}
