...
// NewProductRPCConn instantiate rpc client connection
func NewProductRPCConn() {
	cfg := config.Get()
	...
	var err error
	productConn, err = grpccli.Dial(context.Background(), endpoint, cliOptions...)
	if err != nil {
		panic(fmt.Sprintf("dial rpc server failed: %v, name: %s, endpoint: %s", err, serverName, endpoint))
	}
}

// GetProductRPCConn get client conn
func GetProductRPCConn() *grpc.ClientConn {
	if productConn == nil {
		productOnce.Do(func() {
			NewProductRPCConn()
		})
...
// CloseProductRPCConn Close tears down the ClientConn and all underlying connections.
func CloseProductRPCConn() error {
	if productConn == nil {
		return nil
	}

	return productConn.Close()
}
