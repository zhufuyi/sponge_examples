...
// NewStockRPCConn instantiate rpc client connection
func NewStockRPCConn() {
	cfg := config.Get()
	...
	var err error
	stockConn, err = grpccli.Dial(context.Background(), endpoint, cliOptions...)
	if err != nil {
		panic(fmt.Sprintf("dial rpc server failed: %v, name: %s, endpoint: %s", err, serverName, endpoint))
	}
}

// GetStockRPCConn get client conn
func GetStockRPCConn() *grpc.ClientConn {
	if stockConn == nil {
		stockOnce.Do(func() {
			NewStockRPCConn()
		})
...
// CloseStockRPCConn Close tears down the ClientConn and all underlying connections.
func CloseStockRPCConn() error {
	if stockConn == nil {
		return nil
	}
