...
var (
	couponConn *grpc.ClientConn
	couponOnce sync.Once
)

// NewCouponRPCConn instantiate rpc client connection
func NewCouponRPCConn() {
	cfg := config.Get()
	...
	var err error
	couponConn, err = grpccli.Dial(context.Background(), endpoint, cliOptions...)
	if err != nil {
		panic(fmt.Sprintf("dial rpc server failed: %v, name: %s, endpoint: %s", err, serverName, endpoint))
	}
}

// GetCouponRPCConn get client conn
func GetCouponRPCConn() *grpc.ClientConn {
	if couponConn == nil {
		couponOnce.Do(func() {
			NewCouponRPCConn()
		})
...
// CloseCouponRPCConn Close tears down the ClientConn and all underlying connections.
func CloseCouponRPCConn() error {
	if couponConn == nil {
		return nil
	}

	return couponConn.Close()
}
