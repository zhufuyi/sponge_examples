...
// NewUserRPCConn instantiate rpc client connection
func NewUserRPCConn() {
	cfg := config.Get()
	...
	var err error
	userConn, err = grpccli.Dial(context.Background(), endpoint, cliOptions...)
	if err != nil {
		panic(fmt.Sprintf("dial rpc server failed: %v, name: %s, endpoint: %s", err, serverName, endpoint))
	}
}

// GetUserRPCConn get client conn
func GetUserRPCConn() *grpc.ClientConn {
	if userConn == nil {
		userOnce.Do(func() {
			NewUserRPCConn()
		})
...
// CloseUserRPCConn Close tears down the ClientConn and all underlying connections.
func CloseUserRPCConn() error {
	if userConn == nil {
		return nil
	}
