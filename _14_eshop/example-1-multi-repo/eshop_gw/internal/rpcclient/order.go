...
// NewOrderRPCConn instantiate rpc client connection
func NewOrderRPCConn() {
	cfg := config.Get()
	...
	var err error
	orderConn, err = grpccli.Dial(context.Background(), endpoint, cliOptions...)
	if err != nil {
		panic(fmt.Sprintf("dial rpc server failed: %v, name: %s, endpoint: %s", err, serverName, endpoint))
	}
}

// GetOrderRPCConn get client conn
func GetOrderRPCConn() *grpc.ClientConn {
	if orderConn == nil {
		orderOnce.Do(func() {
			NewOrderRPCConn()
		})
...
// CloseOrderRPCConn Close tears down the ClientConn and all underlying connections.
func CloseOrderRPCConn() error {
	if orderConn == nil {
		return nil
	}
