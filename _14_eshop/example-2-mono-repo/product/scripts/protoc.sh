...
function generateBySpecifiedProto(){
  # get the proto file of the product server
  allProtoFiles=""
  listProtoFiles ${protoBasePath}/product
  cd ..
  specifiedProtoFiles=""
  getSpecifiedProtoFiles
  if [ $? -eq 0 ]; then
  ...
  protoc --proto_path=. --proto_path=./third_party \
    --openapiv2_out=. --openapiv2_opt=logtostderr=true --openapiv2_opt=allow_merge=true --openapiv2_opt=merge_file_name=product/docs/apis.json \
    $specifiedProtoFiles

  checkResult $?

  sponge web swagger --file=product/docs/apis.swagger.json
  checkResult $?

  moduleName=$(cat product/docs/gen.info | head -1 | cut -d , -f 1)
  serverName=$(cat product/docs/gen.info | head -1 | cut -d , -f 2)
  suitedMonoRepo=$(cat product/docs/gen.info | head -1 | cut -d , -f 3)
  ...
  sponge merge rpc-pb --dir=product
  checkResult $?
  ...
  sponge merge http-pb --dir=product
  checkResult $?
  ...
  if [ "$suitedMonoRepo" == "true" ]; then
    sponge patch adapt-mono-repo --dir=product
  fi
}
...
sponge patch modify-dup-num --dir=product/internal/ecode
sponge patch modify-dup-err-code --dir=product/internal/ecode
