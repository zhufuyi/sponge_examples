...
function generateBySpecifiedProto(){
  # get the proto file of the order server
  allProtoFiles=""
  listProtoFiles ${protoBasePath}/order
  cd ..
  specifiedProtoFiles=""
  getSpecifiedProtoFiles
  if [ $? -eq 0 ]; then
  ...
  protoc --proto_path=. --proto_path=./third_party \
    --openapiv2_out=. --openapiv2_opt=logtostderr=true --openapiv2_opt=allow_merge=true --openapiv2_opt=merge_file_name=order/docs/apis.json \
    $specifiedProtoFiles

  checkResult $?

  sponge web swagger --file=order/docs/apis.swagger.json
  checkResult $?

  moduleName=$(cat order/docs/gen.info | head -1 | cut -d , -f 1)
  serverName=$(cat order/docs/gen.info | head -1 | cut -d , -f 2)
  suitedMonoRepo=$(cat order/docs/gen.info | head -1 | cut -d , -f 3)
  ...
  sponge merge http-pb --dir=order
  checkResult $?
  ...
  if [ "$suitedMonoRepo" == "true" ]; then
    sponge patch adapt-mono-repo --dir=order
  fi
}
...
sponge patch modify-dup-num --dir=order/internal/ecode
sponge patch modify-dup-err-code --dir=order/internal/ecode
