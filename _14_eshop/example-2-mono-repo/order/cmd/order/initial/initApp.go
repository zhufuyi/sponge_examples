...
import (
	...
	"eshop/order/internal/notify/receiver"
	"eshop/order/internal/notify/sender"
...
func InitApp() {
	...
	err = sender.InitSyncProducer()
	if err != nil {
		panic(fmt.Sprintf("connect to kafka err, %v", err))
	}
	logger.Info("[kafka producer] was initialized")
	// initializing kafka consumer group
	err = receiver.InitConsumerGroup()
	if err != nil {
		panic(fmt.Sprintf("connect to kafka err, %v", err))
	}
	logger.Info("[kafka consumer group] was initialized")

	rpcclient.InitDtmServerResolver()
	rpcclient.InitEndpointsForDtm()
}
