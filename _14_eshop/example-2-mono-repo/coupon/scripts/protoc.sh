...
function generateBySpecifiedProto(){
  ...
  protoc --proto_path=. --proto_path=./third_party \
    --openapiv2_out=. --openapiv2_opt=logtostderr=true --openapiv2_opt=allow_merge=true --openapiv2_opt=merge_file_name=coupon/docs/apis.json \
    $specifiedProtoFiles

  checkResult $?

  sponge web swagger --file=coupon/docs/apis.swagger.json
  checkResult $?

  moduleName=$(cat coupon/docs/gen.info | head -1 | cut -d , -f 1)
  serverName=$(cat coupon/docs/gen.info | head -1 | cut -d , -f 2)
  suitedMonoRepo=$(cat coupon/docs/gen.info | head -1 | cut -d , -f 3)
  ...
  sponge merge rpc-pb --dir=coupon
  checkResult $?
  ...
  sponge merge http-pb --dir=coupon
  checkResult $?
  ...
  if [ "$suitedMonoRepo" == "true" ]; then
    sponge patch adapt-mono-repo --dir=coupon
  fi
}
...
sponge patch modify-dup-num --dir=coupon/internal/ecode
sponge patch modify-dup-err-code --dir=coupon/internal/ecode
