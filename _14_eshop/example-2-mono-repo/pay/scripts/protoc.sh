...
function generateBySpecifiedProto(){
  # get the proto file of the pay server
  allProtoFiles=""
  listProtoFiles ${protoBasePath}/pay
  cd ..
  specifiedProtoFiles=""
  getSpecifiedProtoFiles
  if [ $? -eq 0 ]; then
  ...
  protoc --proto_path=. --proto_path=./third_party \
    --openapiv2_out=. --openapiv2_opt=logtostderr=true --openapiv2_opt=allow_merge=true --openapiv2_opt=merge_file_name=pay/docs/apis.json \
    $specifiedProtoFiles

  checkResult $?

  sponge web swagger --file=pay/docs/apis.swagger.json
  checkResult $?

  moduleName=$(cat pay/docs/gen.info | head -1 | cut -d , -f 1)
  serverName=$(cat pay/docs/gen.info | head -1 | cut -d , -f 2)
  suitedMonoRepo=$(cat pay/docs/gen.info | head -1 | cut -d , -f 3)
  ...
  if [ "$suitedMonoRepo" == "true" ]; then
    sponge patch adapt-mono-repo --dir=pay
  fi
}
...
sponge patch modify-dup-num --dir=pay/internal/ecode
sponge patch modify-dup-err-code --dir=pay/internal/ecode
