...
function generateBySpecifiedProto(){
  ...
  protoc --proto_path=. --proto_path=./third_party \
    --openapiv2_out=. --openapiv2_opt=logtostderr=true --openapiv2_opt=allow_merge=true --openapiv2_opt=merge_file_name=user/docs/apis.json \
    $specifiedProtoFiles

  checkResult $?

  sponge web swagger --file=user/docs/apis.swagger.json
  checkResult $?

  moduleName=$(cat user/docs/gen.info | head -1 | cut -d , -f 1)
  serverName=$(cat user/docs/gen.info | head -1 | cut -d , -f 2)
  suitedMonoRepo=$(cat user/docs/gen.info | head -1 | cut -d , -f 3)
  ...
  if [ "$suitedMonoRepo" == "true" ]; then
    sponge patch adapt-mono-repo --dir=user
  fi
}
