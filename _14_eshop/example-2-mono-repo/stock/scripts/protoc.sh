...
function generateBySpecifiedProto(){
  # get the proto file of the stock server
  allProtoFiles=""
  listProtoFiles ${protoBasePath}/stock
  cd ..
  specifiedProtoFiles=""
  getSpecifiedProtoFiles
  if [ $? -eq 0 ]; then
  ...
  protoc --proto_path=. --proto_path=./third_party \
    --openapiv2_out=. --openapiv2_opt=logtostderr=true --openapiv2_opt=allow_merge=true --openapiv2_opt=merge_file_name=stock/docs/apis.json \
    $specifiedProtoFiles

  checkResult $?

  sponge web swagger --file=stock/docs/apis.swagger.json
  checkResult $?

  moduleName=$(cat stock/docs/gen.info | head -1 | cut -d , -f 1)
  serverName=$(cat stock/docs/gen.info | head -1 | cut -d , -f 2)
  suitedMonoRepo=$(cat stock/docs/gen.info | head -1 | cut -d , -f 3)
  ...
  sponge merge http-pb --dir=stock
  checkResult $?
  ...
  if [ "$suitedMonoRepo" == "true" ]; then
    sponge patch adapt-mono-repo --dir=stock
  fi
}
...
sponge patch modify-dup-num --dir=stock/internal/ecode
sponge patch modify-dup-err-code --dir=stock/internal/ecode
