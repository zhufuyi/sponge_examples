...
type StockDao interface {
	...
	StockDeduct(tx *sql.Tx, productID uint64, productCount uint32) error
	StockDeductRevert(tx *sql.Tx, productID uint64, productCount uint32) error
	UpdateStockByProductID(ctx context.Context, productID uint64, stock uint32) error
}
...
// StockDeduct 扣减库存
func (d *stockDao) StockDeduct(tx *sql.Tx, productID uint64, productCount uint32) error {
	...
	logger.Info("[mysql] info", logger.String("sql", sqlStr), logger.Any("fieldValues", sqlFieldValues), logger.Int64("affected", affected))
...
func (d *stockDao) StockDeductRevert(tx *sql.Tx, productID uint64, productCount uint32) error {
...
// UpdateStockByProductID 更新库存
func (d *stockDao) UpdateStockByProductID(ctx context.Context, productID uint64, stock uint32) error {
	return d.db.WithContext(ctx).Model(&model.Stock{}).Where("product_id = ?", productID).Update("stock", stock).Error
}
