...
function generateBySpecifiedProto(){
  # get the proto file of the flashSale server
  allProtoFiles=""
  listProtoFiles ${protoBasePath}/flashSale
  cd ..
  specifiedProtoFiles=""
  getSpecifiedProtoFiles
  if [ $? -eq 0 ]; then
  ...
  protoc --proto_path=. --proto_path=./third_party \
    --openapiv2_out=. --openapiv2_opt=logtostderr=true --openapiv2_opt=allow_merge=true --openapiv2_opt=merge_file_name=flashSale/docs/apis.json \
    $specifiedProtoFiles

  checkResult $?

  sponge web swagger --file=flashSale/docs/apis.swagger.json
  checkResult $?

  moduleName=$(cat flashSale/docs/gen.info | head -1 | cut -d , -f 1)
  serverName=$(cat flashSale/docs/gen.info | head -1 | cut -d , -f 2)
  suitedMonoRepo=$(cat flashSale/docs/gen.info | head -1 | cut -d , -f 3)
  ...
  sponge merge rpc-pb --dir=flashSale
  checkResult $?
  ...
  sponge merge http-pb --dir=flashSale
  checkResult $?
  ...
  if [ "$suitedMonoRepo" == "true" ]; then
    sponge patch adapt-mono-repo --dir=flashSale
  fi
}
...
sponge patch modify-dup-num --dir=flashSale/internal/ecode
sponge patch modify-dup-err-code --dir=flashSale/internal/ecode
