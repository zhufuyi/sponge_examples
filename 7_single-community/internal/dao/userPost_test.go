...
func Test_userPostDao_GetByID(t *testing.T) {
	...
	_, err := d.IDao.(UserPostDao).GetByID(d.Ctx, testData.ID) // notfound
	if err != nil {
		t.Fatal(err)
	}
...
func Test_userPostDao_CreateByTx(t *testing.T) {
	...
	err := d.IDao.(UserPostDao).CreateByTx(d.Ctx, d.DB, testData)
	if err != nil {
		t.Fatal(err)
	}
}
...
func Test_userPostDao_DeleteByTx(t *testing.T) {
	...
	err := d.IDao.(UserPostDao).DeleteByTx(d.Ctx, d.DB, testData.ID, 1)
	if err != nil {
		t.Fatal(err)
	}
}
