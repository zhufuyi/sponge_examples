...
var _ CourseDao = (*courseDao)(nil)

// CourseDao defining the dao interface
type CourseDao interface {
	Create(ctx context.Context, table *model.Course) error
	...
	CreateByTx(ctx context.Context, tx *gorm.DB, table *model.Course) (uint64, error)
	UpdateByTx(ctx context.Context, tx *gorm.DB, table *model.Course) error
...
func (d *courseDao) Create(ctx context.Context, table *model.Course) error {
...
func (d *courseDao) DeleteByID(ctx context.Context, id uint64) error {
	err := d.db.WithContext(ctx).Where("id = ?", id).Delete(&model.Course{}).Error
	if err != nil {
		return err
	}
...
func (d *courseDao) DeleteByIDs(ctx context.Context, ids []uint64) error {
	err := d.db.WithContext(ctx).Where("id IN (?)", ids).Delete(&model.Course{}).Error
	if err != nil {
		return err
	}
...
func (d *courseDao) updateByID(ctx context.Context, db *gorm.DB, table *model.Course) error {
	...
	if table.Credit != 0 {
		update["credit"] = table.Credit
	}
	if table.College != "" {
		update["college"] = table.College
	}
	if table.Semester != "" {
		update["semester"] = table.Semester
	}
	if table.Time != "" {
...
func (d *courseDao) GetByID(ctx context.Context, id uint64) (*model.Course, error) {
	...
	if errors.Is(err, model.ErrCacheNotFound) {
		...
		val, err, _ := d.sfg.Do(utils.Uint64ToStr(id), func() (interface{}, error) { //nolint
			...
			// set cache
			err = d.cache.Set(ctx, id, table, cache.CourseExpireTime)
			if err != nil {
	...
	if len(missedIDs) > 0 {
		...
		if len(realMissedIDs) > 0 {
			...
			if len(missedData) > 0 {
				...
				err = d.cache.MultiSet(ctx, missedData, cache.CourseExpireTime)
				if err != nil {
					return nil, err
				}
			} else {
...
//		},
//	}
func (d *courseDao) GetByColumns(ctx context.Context, params *query.Params) ([]*model.Course, int64, error) {
	...
	if params.Sort != "ignore count" { // determine if count is required
		err = d.db.WithContext(ctx).Model(&model.Course{}).Select([]string{"id"}).Where(queryStr, args...).Count(&total).Error
		if err != nil {
			return nil, 0, err
		}
		if total == 0 {
...
func (d *courseDao) CreateByTx(ctx context.Context, tx *gorm.DB, table *model.Course) (uint64, error) {
	err := tx.WithContext(ctx).Create(table).Error
	return table.ID, err
}

// DeleteByTx delete a record by id in the database using the provided transaction
func (d *courseDao) DeleteByTx(ctx context.Context, tx *gorm.DB, id uint64) error {
	update := map[string]interface{}{
		"deleted_at": time.Now(),
	}
	err := tx.WithContext(ctx).Model(&model.Course{}).Where("id = ?", id).Updates(update).Error
	if err != nil {
		return err
	}
