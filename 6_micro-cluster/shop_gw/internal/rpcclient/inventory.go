...
// NewInventoryRPCConn instantiate rpc client connection
func NewInventoryRPCConn() {
	cfg := config.Get()
	...
	var err error
	inventoryConn, err = grpccli.Dial(context.Background(), endpoint, cliOptions...)
	if err != nil {
		panic(fmt.Sprintf("dial rpc server failed: %v, name: %s, endpoint: %s", err, serverName, endpoint))
	}
}

// GetInventoryRPCConn get client conn
func GetInventoryRPCConn() *grpc.ClientConn {
...
// CloseInventoryRPCConn Close tears down the ClientConn and all underlying connections.
func CloseInventoryRPCConn() error {
	if inventoryConn == nil {
		return nil
	}
