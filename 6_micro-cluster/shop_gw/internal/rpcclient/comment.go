...
// NewCommentRPCConn instantiate rpc client connection
func NewCommentRPCConn() {
	cfg := config.Get()
	...
	var err error
	commentConn, err = grpccli.Dial(context.Background(), endpoint, cliOptions...)
	if err != nil {
		panic(fmt.Sprintf("dial rpc server failed: %v, name: %s, endpoint: %s", err, serverName, endpoint))
	}
}

// GetCommentRPCConn get client conn
func GetCommentRPCConn() *grpc.ClientConn {
	if commentConn == nil {
		commentOnce.Do(func() {
			NewCommentRPCConn()
		})
...
// CloseCommentRPCConn Close tears down the ClientConn and all underlying connections.
func CloseCommentRPCConn() error {
	if commentConn == nil {
		return nil
	}

	return commentConn.Close()
}
