...
function generateBySpecifiedProto(){
  ...
  moduleName=$(cat comment/docs/gen.info | head -1 | cut -d , -f 1)
  serverName=$(cat comment/docs/gen.info | head -1 | cut -d , -f 2)
  suitedMonoRepo=$(cat comment/docs/gen.info | head -1 | cut -d , -f 3)
  ...
  sponge merge rpc-pb --dir=comment
  checkResult $?
  ...
  if [ "$suitedMonoRepo" == "true" ]; then
    sponge patch adapt-mono-repo --dir=comment
  fi
}
...
sponge patch modify-dup-num --dir=comment/internal/ecode
sponge patch modify-dup-err-code --dir=comment/internal/ecode
