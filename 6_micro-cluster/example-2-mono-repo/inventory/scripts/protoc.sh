...
function generateBySpecifiedProto(){
  # get the proto file of the inventory server
  allProtoFiles=""
  listProtoFiles ${protoBasePath}/inventory
  cd ..
  specifiedProtoFiles=""
  getSpecifiedProtoFiles
  if [ $? -eq 0 ]; then
  ...
  moduleName=$(cat inventory/docs/gen.info | head -1 | cut -d , -f 1)
  serverName=$(cat inventory/docs/gen.info | head -1 | cut -d , -f 2)
  suitedMonoRepo=$(cat inventory/docs/gen.info | head -1 | cut -d , -f 3)
  ...
  sponge merge rpc-pb --dir=inventory
  checkResult $?
  ...
  if [ "$suitedMonoRepo" == "true" ]; then
    sponge patch adapt-mono-repo --dir=inventory
  fi
}
...
sponge patch modify-dup-num --dir=inventory/internal/ecode
sponge patch modify-dup-err-code --dir=inventory/internal/ecode
