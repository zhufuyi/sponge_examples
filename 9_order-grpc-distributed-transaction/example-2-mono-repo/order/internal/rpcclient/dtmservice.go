...
func InitDtmServerResolver() {
	...
	if isUseDiscover {
		logger.Infof("connect [dtm] using service discovery, type = %s, endpoint = %s", grpcClientCfg.RegistryDiscoveryType, dtmServerEndPoint)
		builder := discovery.NewBuilder(iDiscovery, discovery.WithInsecure(true), discovery.DisableDebugLog())
		resolver.Register(builder)
	} else {
		logger.Infof("connect [dtm] directly, endpoint = %s", dtmServerEndPoint)
	}
}
